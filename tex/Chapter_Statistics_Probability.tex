...
\end{align*}
$P(result|p)$ is simply the binomial probability of obtaining 7 heads out of 10 tosses for a given value of $p$. $P(result)$ is the probability of obtaining 7 out of 10 results, regardless of the value of $p$. Since it does not depend on $p$, we can ignore it, but it is simply given by:
\begin{align*}
P(result) = \int_0^1P(result|p)dp
\end{align*}
...
\end{align*}
That is, we can treat the binomial probability as the probability density for $p$, given a fixed value of $k$ and $N$. Using this flat prior, the posterior probability for $p$ can be calculated with the simple python code, and is plotted in Figure \ref{fig:coin_post_flat}.
...
\capfig{0.4\textwidth}{figures/coin_post_flat.png}{\label{fig:coin_post_flat} Posterior probability for the probability that a coin will land on heads, based on having had the result of 7 heads in 10 coin tosses and a flat prior.}

Not surprisingly, the posterior probability for $p$ is peaked at $p=0.7$, however the distribution is quite broad, indicating the the data do not place a very strong constraint on the value of $p$. We can use this posterior probability for $p$ to give our degree of belief that $p$ lies within a certain range. Note that the posterior probability is not normalized. The python code below gives the probability that $p$ is between 0.45 and 0.55:
\begin{lstlisting}[frame=single] 
...
\capfig{0.4\textwidth}{figures/fakexy.png}{\label{fig:fakexy} Simulated data generated with an offset of 3 and a slope of 8.}
...
\capfig{0.4\textwidth}{figures/posterior2d.png}{\label{fig:posterior2d} 2-dimensional posterior distribution, for the case with $\mu_b=10$ and $\sigma_b=1$.}
