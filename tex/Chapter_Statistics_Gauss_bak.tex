...
P_x(x)&=\frac{1}{\sigma_x\sqrt{2\pi}}e^{-\frac{(x-\mu_x)^2}{2\sigma_x^2}}\\
\end{align*}
and we want to know the probability density function, $P_F(F)$. Since $F=ax$, the probability, $P_F(F)dF$, of obtaining a certain value of $F$ between $F$ and $F+dF$ is the same as that for obtaining the corresponding value of $x$, between $x=\frac{F}{a}$ and $x=\frac{F}{a}+dF=\frac{F}{a}+\frac{dF}{dx}dx = \frac{F}{a}+adx$ (since  $\frac{dF}{dx}=1$). The probability of obtaining $F$ between $F$ and $F+dF$ is thus equal to the probability of obtaining $x$ between $\frac{F}{a}$ and $\frac{F}{a}+adx$

 The probability is given by:
\begin{align}
P_F(F)dF=P_x(x=\frac{F}{a})\frac{1}{a}dx&=\frac{1}{\sigma_x\sqrt{2\pi}}e^{-\frac{(\frac{F}{a}-\mu_x)^2}{2\sigma_x^2}}dx \nonumber\\
