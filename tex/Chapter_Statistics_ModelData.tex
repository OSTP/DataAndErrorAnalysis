...
\begin{align*}
L(x,y|m,b) = P(x_1,x_2,\dots,x_N,y_1,y_2,\dots,y_N|m,b)\propto\prod_{i=1}^{i=N}\frac{1}{\sigma_{yi}}e^{-\frac{(y_i-y^{exp}(x_i))^2}{2\sigma_{yi}^2}}
\end{align*}
where we have ignored the normalization constant $\frac{1}{\sigma_{yi}\sqrt{2\pi}}$ since it does not depend on $m$ and $b$ and we are only interested in the maximal value. As we did in the previous chapter, we can take the negative of the logarithm of the likelihood (which we will then want to minimize):
\begin{align}
-\ln{L(x,y|m,b)}&=\sum_{i=1}^{i=N}\ln(\sigma_{yi})+\frac{1}{2}\sum_{i=1}^{i=N}\frac{(y_i-y^{exp}(x_i))^2}{\sigma_{yi}^2}
\end{align}
...
\chi^2=\sum_{i=1}^{i=N}\frac{(y_i-y^{exp}(x_i,\vec\beta))^2}{\sigma_{y_i}^2}
\end{align}
which is a sum of the squared distance between the points and the model, divided by the error in the data points. For this reason, the method is also called the ``Method of Least Squares''. We note that this \textbf{method is only valid when we expect the errors on the $y$ points to standard} (i.e. that the uncertainties on the $y$ values correspond to the standard deviation of values of that are normally distributed about the model prediction, $y^{exp}$).
